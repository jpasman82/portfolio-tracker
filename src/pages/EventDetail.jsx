...
export default function EventDetail() {
  ...
  const save = async (close = false) => {
    ...
    try {
      await updateDoc(doc(db, "rotations", id), { 
        ...
        isClosed: close ? true : (event.isClosed || false) 
      });
      setIsEditingStructure(false); 
      alert(close ? "Cerrada" : "Guardado");
  ...
  if (!event) return <div style={{ padding: '100px', textAlign: 'center' }}>No encontrada.</div>;
  ...
  return (
    <div style={{ padding: '24px 15px', maxWidth: '500px', margin: 'auto', paddingBottom: '120px' }}>
      ...
      <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '24px', border: '1px solid #eee', marginBottom: '20px', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
          <div style={{ borderRight: '1px solid #f0f0f0', paddingRight: '10px' }}>
            ...
            <div style={{ fontSize: '16px', fontWeight: 800 }}>$ {totalARS_Init.toLocaleString('es-AR', {maximumFractionDigits: 0})}</div>
          ...
          <div>
            ...
            <div style={{ fontSize: '16px', fontWeight: 800 }}>$ {totalARS_Now.toLocaleString('es-AR', {maximumFractionDigits: 0})}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '20px' }}>
        <div style={{ borderRadius: '16px', padding: '12px 5px', textAlign: 'center', backgroundColor: pUSD > 0 ? '#00c805' : '#ff3b30', color: 'white' }}>
          <div style={{ fontSize: '8px', fontWeight: 700 }}>REND. USD</div><div style={{ fontSize: '16px', fontWeight: 800 }}>{pUSD.toFixed(1)}%</div>
        </div>
        <div style={{ borderRadius: '16px', padding: '12px 5px', textAlign: 'center', backgroundColor: pARS > 0 ? '#00c805' : '#ff3b30', color: 'white' }}>
          <div style={{ fontSize: '8px', fontWeight: 700 }}>REND. ARS</div><div style={{ fontSize: '16px', fontWeight: 800 }}>{pARS.toFixed(1)}%</div>
        </div>
        <div style={{ borderRadius: '16px', padding: '12px 5px', textAlign: 'center', backgroundColor: pALFA > 0 ? '#00c805' : '#ff3b30', color: 'white' }}>
          <div style={{ fontSize: '8px', fontWeight: 700 }}>ALFA</div><div style={{ fontSize: '16px', fontWeight: 800 }}>{pALFA.toFixed(1)}%</div>
        </div>
      </div>
      ...
      <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '24px', border: '1px solid #eaecef', marginBottom: '15px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
          ...
          {!event.isClosed && !isEditingStructure && <button onClick={() => setIsEditingStructure(true)} style={{ border: 'none', background: '#f8f9fa', color: '#0d6efd', fontWeight: 700, fontSize: '11px', padding: '5px 10px' }}>✏️ Editar</button>}
        </div>
        {currentAssets.map((asset) => (
          <div key={asset.ticker} style={{ marginBottom: '15px', borderBottom: '1px solid #f8f9fa' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: 5 }}>
              <div><label style={{ fontSize: '9px' }}>CANTIDAD</label><input type="number" value={asset.quantity} disabled={!isEditingStructure} onChange={(e) => setCurrentAssets(currentAssets.map(a => a.ticker === asset.ticker ? {...a, quantity: Number(e.target.value)} : a))} style={{ width: '100%', padding: '8px', border: '1px solid #eee' }} /></div>
              <div><label style={{ fontSize: '9px' }}>PRECIO ARS</label><input type="number" value={currentPrices[asset.ticker]} disabled={event.isClosed} onChange={(e) => setCurrentPrices({...currentPrices, [asset.ticker]: Number(e.target.value)})} style={{ width: '100%', padding: '8px', border: '1px solid #eee', textAlign: 'right' }} /></div>
            </div>
          </div>
        ))}
      </div>
      ...
      {!event.isClosed && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <button onClick={() => save(false)} disabled={saving} style={{ width: '100%', padding: '18px', backgroundColor: '#1a1d21', color: 'white', border: 'none', borderRadius: '16px', fontWeight: 700 }}>Guardar Cambios</button>
          <button onClick={() => save(true)} style={{ color: '#dc3545', border: 'none', background: 'none', fontSize: '13px' }}>Cerrar Operación</button>
        </div>
      )}
    </div>
  );
