...
export default function Home() {
    ...
    const fetchBalances = async () => {
      try {
        ...
        if (latestDate > 0) {
          ...
          setLastUpdated(`${d.getDate()}/${d.getMonth()+1} a las ${d.getHours()}:${d.getMinutes().toString().padStart(2, '0')}hs`);
        } else {
  ...
  const brokers = [
    { id: 'jpm', name: 'JP Morgan', balance: balances.jpm, logo: 'https://upload.wikimedia.org/wikipedia/commons/a/af/J_P_Morgan_Chase_Logo_2008-1.svg' },
  ...
  return (
    <div style={{ padding: '24px 15px', maxWidth: '600px', margin: 'auto', backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '30px 20px', backgroundColor: '#1a1d21', borderRadius: '28px', marginBottom: '25px', color: 'white', position: 'relative' }}>
        <div style={{ fontSize: '13px', fontWeight: 700, opacity: 0.8, textTransform: 'uppercase', marginBottom: '8px' }}>Total Consolidado</div>
        <div style={{ fontSize: '38px', fontWeight: 900 }}>US$ {totalConsolidado.toLocaleString('en-US', { maximumFractionDigits: 0 })}</div>
        
        {lastUpdated && (
          <div style={{ position: 'absolute', top: '25px', right: '20px', fontSize: '10px', fontWeight: 800, backgroundColor: 'rgba(255,255,255,0.1)', padding: '5px 10px', borderRadius: '10px' }}>
            ACT: {lastUpdated}
          </div>
        )}
      </div>
            ...
            <div style={{ padding: '20px 18px', backgroundColor: 'white', borderRadius: '20px', border: '1px solid #eaecef', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '12px', color: '#0d6efd', fontWeight: 700 }}>Editar posición →</div>
                </div>
              </div>
