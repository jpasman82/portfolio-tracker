...
async function getGoogleAccessToken() {
  ...
  if (!response.ok) throw Object.assign(new Error('GOOGLE_AUTH_FAILED'), { code: 'GOOGLE_AUTH_FAILED', httpStatus: response.status });
...
export async function updatePositionsAndBuildSnapshot(input) {
  const nowIso = input?.capturedAt || new Date().toISOString();
  const { prices, usdBondSymbols, mep, cable } = input?.marketData || await fetchBymaPrices();
  const positions = input?.positions || await listBrokerPositions();
  ...
  positions.forEach((position) => {
    ...
    const updatedAssets = (data.assets || []).map((asset) => {
      ...
      const livePrice = input?.priceForAsset
        ? input.priceForAsset(brokerId, ticker)
        : getLivePrice(ticker, prices, { isUSD, mep, usdBondSymbols });
      const isBond = asset.isBond || isBondTicker(ticker);
      if (!BRAZIL_CEDEARS.has(ticker) && livePrice === undefined
        && (!input || parseNum(asset.quantity) !== 0)) missingPrices.add(ticker);
    ...
    for (const asset of updatedAssets) {
      ...
      const marketKey = input ? `${asset.ticker}:${isUSD ? 'USD' : 'ARS'}` : asset.ticker;
      if (!marketPriceByTicker[marketKey]) {
        marketPriceByTicker[marketKey] = {
          ticker: asset.ticker,
          isBond,
      ...
      marketPriceByTicker[marketKey].quantity += quantity;
      marketPriceByTicker[marketKey].valueUsd += valueUsd;
      marketPriceByTicker[marketKey].valueArs += valueArs;
      marketPriceByTicker[marketKey].brokers.push(brokerId);
    }
  ...
  return {
    date: input?.valuationDate || snapshotDate(),
...
// B1 reuses only the baseline credential/codec helpers and valuation arithmetic.
// The legacy handler is retained solely for explicit rollback; never used as a
// fallback after a B1 error. B1 never executes positionUpdates.
export { getGoogleAccessToken, getServiceAccount, encodeFirestoreValue,
  decodeFirestoreFields, snapshotDate, isWeekdayInArgentina };
